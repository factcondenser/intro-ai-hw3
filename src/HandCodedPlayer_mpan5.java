 ...
 *
 */
public class HandCodedPlayer_mpan5 extends NannonPlayer {

	/** 
	 * You should rename this file to:  HandCodedPlayer_yourLoginName.java
	 * 
	 */
	public HandCodedPlayer_mpan5() {
	}

	public HandCodedPlayer_mpan5(NannonGameBoard gameBoard) {
		super(gameBoard);
	}
	@Override
	public String getPlayerName() {
		...
		return "mpan5's Hand-Coded Player";
	}
	...
	public List<Integer> chooseMove(int[] boardConfiguration, List<List<Integer>> legalMoves) {
		...
		/* Some code taken from RandomNannonPlayer.
			...
			int fromCountingFromOne   = move.get(0); // A board position or NannonGameBoard.movingFromHOME.
			int   toCountingFromOne   = move.get(1); // A board position or NannonGameBoard.movingToSAFE
		...
		// Note: when only one piece left, all moves are forced.  So no need to check for a move that wins the game (because this method is only called when there is a choice to be made).
		
		// First try to move pieces to SAFETY but don't ruin a prime.
		for (List<Integer> move : legalMoves) if (move.get(1) == NannonGameBoard.movingToSAFETY && !ManageMoveEffects.breaksPrime(move.get(2))) {
			return move; // Take first such move found.
		}
		
		// Next try to create or extend a prime AND hit opponent.		
		for (List<Integer> move : legalMoves) if ( ManageMoveEffects.isaHit(      move.get(2)) &&
												  (ManageMoveEffects.createsPrime(move.get(2)) || ManageMoveEffects.extendsPrime(move.get(2)))) {
			return move;
		}
		
		// If nothing found yet, try to create or extend a prime.		
		for (List<Integer> move : legalMoves) if (ManageMoveEffects.createsPrime(move.get(2))  || ManageMoveEffects.extendsPrime(move.get(2))) {
			return move;
		}
		...
		// Prefer moving from HOME (this won't break a prime since coming from off the board).
		for (List<Integer> move : legalMoves) if (move.get(0) == NannonGameBoard.movingFromHOME) {
			return move;
		}

		// Make a move that doesn't break a prime.
		for (List<Integer> move : legalMoves) if (!ManageMoveEffects.breaksPrime(move.get(2))) {
			return move;
		}

		// Move to SAFETY if you can and no good move, even though a prime will be broken.
		for (List<Integer> move : legalMoves) if (move.get(1) == NannonGameBoard.movingToSAFETY) {
			return move;
		}
