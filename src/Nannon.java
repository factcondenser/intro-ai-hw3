...
//		 	return new JWS_FullJointDistribution_Player(gb); // Can handle 3 pieces per side and 6 board cells, as well as 4 and 6, but runs out of RAM with 5 and 10 (and that is on a machine with 50GB RAM).
			...
			return new HandCodedPlayer_mpan5(gb);	
	...
	public static void main(String[] argsRaw) {
		...
		if (!"shavlik".equals(Utils.getUserName()) && !"jshavlik".equals(Utils.getUserName()) && 
			!"nbridle".equals(Utils.getUserName()) && !"bridle".equals(  Utils.getUserName())) {
			Utils.error("This main is for Jude and Nick to do tests, debug, etc.   Students should use PlayNannon's main().");
			return;
		}		
	...
	//	hiddenNameToUseForO = "Bayesian Network";
		...
		setNumberOfGamesInBurnInPhase(1000 * 1000);
